...
type (
	// MacroKeyword is a GX keyword implemented by a macro.
	// Transforms an IR expression into another.
	// These macros are not visible from the interpreter because
	// the IR is transformed as soon as the macro is seen by the compiler.
	MacroKeyword struct {
		MetaCore

		BuildSynthetic MacroKeywordImpl
	}
