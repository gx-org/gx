...
var tests = []struct{
  ...
  test func(*testing.T)
	...
	{name: "TestImportsReturnScalar", test: TestImportsReturnScalar},
	{name: "TestAddPrivate", test: TestAddPrivate},
...
func Run(t *testing.T, rtm *api.Runtime) {
	...
	err := setupTest(rtm)
	if err != nil {
		t.Errorf("cannot run importtest tests: %+v", err)
		return
	}
	for _, test := range tests {
		t.Run("importtest."+test.name, test.test)
	}
}
