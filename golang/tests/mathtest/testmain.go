...
var tests = []struct{
  ...
  test func(*testing.T)
	...
	{name: "TestMathFloat32", test: TestMathFloat32},
	{name: "TestMathFloat64", test: TestMathFloat64},
...
func Run(t *testing.T, rtm *api.Runtime) {
	...
	err := setupTest(rtm)
	if err != nil {
		t.Errorf("cannot run mathtest tests: %+v", err)
		return
	}
	for _, test := range tests {
		t.Run("mathtest."+test.name, test.test)
	}
}
