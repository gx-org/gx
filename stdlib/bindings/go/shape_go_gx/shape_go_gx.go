...
// Package shape_go_gx are Go bindings to the GX package:
// shape.
//
...
// BuildFor loads the GX package shape
...
// BuildHandleFor loads the GX package shape
...
func Build(dev *core.DeviceSetup) (*PackageHandle, error) {
	bpkg, err := dev.Runtime().Builder().Build("shape")
	if err != nil {
		return nil, err
	}
	pkg := &Package{}
