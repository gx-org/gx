...
func (g goGenerator) Generate() (string, error) {
	g.file = genast.NewFile(g.target.GoPkgName)
	gxSource := g.cfg.GenFileName() + ".gx"
	...
	g.file.FuncDecl("init", nil, genast.NewBlock(genast.CallStmt(
		...
		genast.StringLit(g.cfg.GoImportPath(g.target.Src.Path())),
	...
	buildPackage := g.file.Var("buildPackage", nil, genast.StructLit(
		...
		genast.KeyVal("FullPath", genast.StringLit(g.target.GxPkgPath())),
