...
type builder[T dtypes.GoDataType] struct {
	w       *strings.Builder
	data    []T
	axes    []int
	offsets []int
...
func newBuilder[T dtypes.GoDataType](data []T, axes []int) (*builder[T], error) {
	b := &builder[T]{
		w:       &strings.Builder{},
		data:    data,
		axes:    axes,
...
func SDataPrint[T dtypes.GoDataType](data []T, axes []int) string {
...
func Sprint[T dtypes.GoDataType](data []T, axes []int) string {
	b, err := newBuilder[T](data, axes)
	if err != nil {
		return err.Error()
	}
